...
type Lambda = {
  fetch: (request: Request, server: Server) => Promise<Response | undefined | APIGatewayAuthorizerResult>;
...
// reference for APIGatewayAuthorizerResultContext from aws-lambda
type AuthorizerResultContext = {
...
type APIGatewayAuthorizerResult = {
  ...
  policyDocument: {
    Version: string;
    Id?: string | undefined;
    Statement: any[]; // API Gateway CustomAuthorizer AuthResponse.PolicyDocument.Statement.
  };
  context?: AuthorizerResultContext | null | undefined;
...
async function receiveRequest(): Promise<LambdaRequest> {
  const response = await fetch("runtime/invocation/next") as Response;
  ...
  traceId = response.headers.get("Lambda-Runtime-Trace-Id") ?? "local";
...
function formatRequest(input: LambdaRequest): Request | undefined {
  ...
  if(event.requestContext?.authorizer) {
    request.headers.set("x-amzn-authorizer", JSON.stringify(event.requestContext.authorizer));
  }
...
class LambdaServer implements Server {
  ...
  async #acceptRequest(event: LambdaRequest): Promise<unknown> {
    ...
    if (request === undefined) {
      ...
      if (event.event?.methodArn) {
        const authResponse = await this.authorizerFetch(request);
  ...
  isAuthorizer(response: any): boolean {
    return "principalId" in response && "policyDocument" in response;
  }
  ...
  async authorizerFetch(request: Request): Promise<APIGatewayAuthorizerResult | Response> {
    this.pendingRequests++;
    try {
      let response = await this.#lambda.fetch(request, this);
